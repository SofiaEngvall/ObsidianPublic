...
__export(main_exports, {
  default: () => PlaintextPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian5 = require("obsidian");

// src/helper.ts
var OBSIDIAN_EXTENSIONS = /* @__PURE__ */ new Set([
  "md",
  "png",
  "webp",
  "jpg",
...
var PROBLEMATIC_PLUGINS = /* @__PURE__ */ new Map([
...
var removeObsidianExtensions = (exts) => {
  return exts.filter((ext) => !OBSIDIAN_EXTENSIONS.has(ext));
};
var removeOtherExtensions = (exts, enabledPlugins) => {
...
var craftLogMessage = (plugin, message) => {
...
var PlaintextNotice = class extends import_obsidian.Notice {
...
var processExts = async (_this) => {
  let currentExtensionList = Array.from(_this.plugin.settings.extensions);
  currentExtensionList = currentExtensionList == null || currentExtensionList == void 0 ? [] : Array.from(new Set(currentExtensionList));
  let newExtensionList = _this.changes == null || _this.changes == void 0 ? [] : _this.changes.split(",").map((s) => s.toLowerCase().trim()).filter((s) => s != "");
...
var PlaintextSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Extensions").setDesc(
      ...
      text.setPlaceholder("Extensions").setValue(
    ...
    new import_obsidian2.Setting(containerEl).setName("Destroy Other Plugins").setDesc(
      "There may be other plugins that already have registered extensions. By turning this setting ON, you willingly disregard those plugins, and will highly likely break them. **ONLY TURN THIS ON IF YOU KNOW WHAT YOU'RE DOING!**"
    ).addToggle((toggle) => {
      toggle.setValue(
...
var ruleNodeProp = new import_common.NodeProp();
var Rule = class {
...
var newlines = new import_lr.ExternalTokenizer((input, stack) => {
  let prev;
  if (input.next < 0) {
...
var indentation = new import_lr.ExternalTokenizer((input, stack) => {
...
var trackIndent = new import_lr.ContextTracker({
  start: topIndent,
  reduce(context, term) {
...
var legacyPrint = new import_lr.ExternalTokenizer((input) => {
  for (let i = 0; i < 5; i++) {
...
var obsidianHighlightStyle = import_language3.HighlightStyle.define([
  { tag: tags.keyword, color: codemirrorConfig.keyword },
  {
    tag: [tags.name, tags.deleted, tags.character, tags.macroName],
    color: codemirrorConfig.variable
  },
  { tag: [tags.propertyName], color: codemirrorConfig.function },
  {
    tag: [
  ...
  { tag: [tags.className], color: codemirrorConfig.class },
  {
    tag: [
      tags.number,
      tags.changed,
  ...
  { tag: [tags.operator, tags.operatorKeyword], color: codemirrorConfig.keyword },
  {
    tag: [tags.url, tags.escape, tags.regexp, tags.link],
    color: codemirrorConfig.regexp
  },
  { tag: [tags.meta, tags.comment], color: codemirrorConfig.comment },
  {
    tag: [tags.atom, tags.bool, tags.special(tags.variableName)],
    color: codemirrorConfig.variable
  },
  { tag: tags.invalid, color: codemirrorConfig.invalid }
]);
var basicExtensions = [
...
var languageExtension = [language.of(python())];
var LANGUAGES = /* @__PURE__ */ new Map([["py", python()]]);

// src/view.ts
var PlaintextView = class extends import_obsidian3.TextFileView {
  constructor(leaf) {
    super(leaf);
    this.editorState = import_state2.EditorState.create({
      extensions: [
        basicExtensions,
...
var CreateNewPlaintextFileModal = class extends import_obsidian4.Modal {
  constructor(app2, defaultFilename, onSubmit) {
    super(app2);
  ...
  onOpen() {
    ...
    new import_obsidian4.Setting(contentEl).setName("File Name (include extension!)").addText(
      (text) => text.setValue(this.filename).onChange((value) => {
        this.filename = value;
      })
    );
    new import_obsidian4.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Create").setCta().onClick(() => this.createFile(this.filename))
    );
...
// src/main.ts
var PlaintextPlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.createNewFile = async (file) => {
      console.log(craftLogMessage(this, "Create new plaintext file"));
      new CreateNewPlaintextFileModal(
        this.app,
        "plaintext file.txt",
        async (res) => {
          const filename = (0, import_obsidian5.normalizePath)(`${file.path}/${res}`);
          let newFile = null;
          try {
