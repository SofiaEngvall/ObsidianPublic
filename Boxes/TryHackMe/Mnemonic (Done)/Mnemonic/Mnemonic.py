...
print("C:\Obsidian\InfoSec\Boxes\TryHackMe\Mnemonic\Mnemonic\maxresdefault.jpg")
print("")

while True:
    gelen = input("Access Code image file Path:")
    if os.path.isfile(gelen):
...
print("Image Analysis Completed Successfully. Your Special Code:")
print(sayilar)
...
def sifrele():
    ...
    if x == x:
        key = [(x // (10 ** i)) %
               10 for i in range(math.ceil(math.log(x, 10)) - 1, -1, -1)]
    toplam = 0
    for letter in key:
...
def coz():
    with open('0.txt', 'r')as f:
        ...
        x123 = int(f_conte)
    print("ENCRYPT Message to file Path'")
    Metingiris = 1
    if Metingiris == 1:
        pass
    ...
    if Metingiris == 1:
        ...
        for i in Tamkilit:
            Coz.append(int(i))
    if Metingiris == 2:
        Tamkilit = input("Please enter the text to be solved:")

        if Tamkilit.isdigit():
            pass
        else:
            print(
                "Incorrect Text Entered Encrypted text cannot contain letters.Please enter again:")
            coz()
    ...
    key = [(x123 // (10 ** i)) %
           10 for i in range(math.ceil(math.log(x123, 10)) - 1, -1, -1)]
    toplam = 0
...
while Devam == "E" or Devam == "e":
    ...
    print(" ")
    print(" ")
    Devam = input("PRESS TO QUİT 'ENTER' OR 'E' PRESS TO CONTİNUE.")
