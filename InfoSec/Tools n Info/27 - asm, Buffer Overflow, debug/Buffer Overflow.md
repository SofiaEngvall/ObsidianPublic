...
When we get a program to crash from sending it more input than it can handle there are a few thing we need to think about.

Our goal is to get control over the Instruction Pointer. To do this we need to overwrite the section of the stack that contains the address the next return instruction will jump to.

A crash does not automatically mean we overwrote the Instruction Pointer. We might have overwritten something else that made the program crash.

When you get a crash from an overflow, check:
- Did we overwrite EIP? We might need to add more data. (Could we have overwritten EIP with less data?)
- How much space do we have for possible shellcode. If we input more/too much data we'll eventually get another type of crash. (Making us lose the control over the Instruction Pointer.)
...
An example here: [[fuzzer.py]]

In this script we can set the size of the steps we take when testing and it can do really big numbers.

The interesting thing is to check:
- Did the program crash?
- Was the Instruction Pointer overwritten?
- Did all data end up on the stack?
